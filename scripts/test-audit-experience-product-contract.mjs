...
expect(frontendAudit.includes('max-w-[1280px]') && frontendAudit.includes('xl:grid-cols-[minmax(0,1fr)_300px]') && frontendAudit.includes('font-lora text-[30px]') && frontendAudit.includes('font-lora text-[26px]'), 'Audit shares the Data Upload operational workspace frame and reserves Lora for principal review headings');
expect(!auditHeader.includes('openAuditLog') && !auditHeader.includes('openScheduleDialog') && auditHeader.includes('title="Margin home"') && auditHeader.includes('Use Amazon reports') && auditHeader.includes('Export summary'), 'Audit shares the Data Upload contextual header with only brand, orientation, report intake, and export');
...
expect(!frontendAudit.includes('sidebarOpen') && !frontendAudit.includes('PanelLeft') && frontendAudit.includes('Audit context') && frontendAudit.includes('Audit controls') && frontendAudit.includes('xl:sticky xl:top-20'), 'Audit replaces the competing rail with the Data Upload-style sticky context column and compact controls');
...
expect(frontendAudit.includes('Your review scope') && frontendAudit.includes('Recovery choices') && !frontendAudit.includes('Continuous Intelligence') && !frontendAudit.includes('Choose how Margin should help.'), 'Audit result review replaces the rejected inline commercial block with a bounded recorded-result decision surface');
...
expect(!frontendAudit.includes('ShieldCheck') && !frontendAudit.includes('RECOVERY AUDIT') && !frontendAudit.includes('LATEST AUDIT RECORD') && !frontendAudit.includes('SELECTED AUDIT RECORD') && !frontendAudit.includes('RECORDED AUDIT REVIEW'), 'Audit removes the shield-icon eyebrow badges and their wide-tracked record labels');
expect(!frontendAudit.includes('tracking-widest') && !frontendAudit.includes('tracking-[0.12em]') && !frontendAudit.includes('tracking-[0.11em]') && !frontendAudit.includes('tracking-[0.1em]'), 'Audit removes wide letter-spacing utilities from the operational surface');
expect(frontendAudit.includes('<SheetContent side="bottom"') && frontendAudit.includes('h-[76vh]') && frontendAudit.includes('sm:w-[calc(100vw-48px)]') && frontendAudit.includes('sm:max-w-[960px]'), 'Recovery Workspace opens as a responsive bottom-origin near-half-screen review sheet');
expect(frontendAudit.includes('rounded-[10px] border border-[#D7D7D1] bg-[#F4F3ED] p-4') && frontendAudit.includes('bg-[#3F51A8] px-4 text-[13px] font-semibold text-white'), 'Recovery bottom sheet uses the Data Upload light-surface and decisive-action treatment');
