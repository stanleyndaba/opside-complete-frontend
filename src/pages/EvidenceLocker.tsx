...
export default function EvidenceLocker() {
  ...
  const [recordedRecoveries, setRecordedRecoveries] = useState<LinkedRecovery[]>([]);
  ...
  useEffect(() => {
    ...
    setRecordedRecoveries([]);
    setCandidateRecoveries([]);
    setAuditEvents([]);
    ...
    void Promise.all([
      ...
      if (linksResponse.ok && linksResponse.data) {
        setRecordedRecoveries(linksResponse.data.linkedClaims || []);
        setCandidateRecoveries(linksResponse.data.candidateMatches || []);
      }
      ...
      if (!cancelled) {
        toast({
          ...
          description: 'The stored artifact remains available. Try refreshing this record later for relationship and candidate details.',
          variant: 'destructive',
        });
      }
    }).finally(() => {
            ...
            return (
              <div className="flex min-h-full flex-col">
                ...
                <div className="flex-1 space-y-6 px-5 py-5 sm:px-6">
                  ...
                  <section><div className="flex items-center justify-between gap-3"><div><p className="text-[12px] font-medium tracking-tight text-[#66737F]">Recovery relationship record</p><h3 className="mt-1 text-[16px] font-semibold tracking-tight text-[#182026]">Recorded links</h3></div><Layers className="h-4 w-4 text-[#66737F]" /></div>{detailLoading ? <p className="mt-4 text-[12px] text-[#66737F]">Loading recorded links and candidate matches…</p> : recordedRecoveries.length > 0 ? <div className="mt-4 divide-y divide-[#E7EEF2] overflow-hidden rounded-[8px] border border-[#DCE8EE]">{recordedRecoveries.map((recovery) => <div key={recovery.claimId} className="flex items-center justify-between gap-3 p-3"><div className="min-w-0"><p className="truncate text-[12px] font-semibold text-[#182026]">{recovery.claimNumber || 'Recovery reference unavailable'}</p><p className="mt-1 text-[10px] leading-4 text-[#66737F]">Recorded relationship: {String(recovery.matchType || 'linked').replace(/_/g, ' ')}{recovery.confidence != null ? ` · ${(recovery.confidence * 100).toFixed(0)}% matching confidence` : ''}</p></div><Button asChild variant="outline" size="sm" className="h-8 shrink-0 border-[#DCE8EE] bg-white px-2 text-[10px] text-[#4D5B66]"><Link to={tenantRoute(activeSlug, `/recoveries/${recovery.claimId}`)}>Open recovery<ArrowRight className="ml-1 h-3 w-3" /></Link></Button></div>)}</div> : <div className="mt-4 rounded-[8px] border border-[#DCE8EE] bg-white p-4"><p className="text-[12px] font-medium text-[#182026]">No recovery relationship recorded</p><p className="mt-1 text-[11px] leading-5 text-[#66737F]">Margin retains the artifact, but no recorded recovery link is available from the current evidence contract.</p></div>}{candidateRecoveries.length > 0 ? <div className="mt-4"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">Candidate recovery matches</p><p className="mt-1 text-[11px] leading-5 text-[#66737F]">These detection-based candidates are not recorded evidence relationships and do not establish proof or a recovery outcome.</p><div className="mt-3 divide-y divide-[#E7EEF2] overflow-hidden rounded-[8px] border border-[#DCE8EE]">{candidateRecoveries.map((candidate) => <div key={candidate.claimId} className="flex items-center justify-between gap-3 p-3"><div className="min-w-0"><p className="truncate text-[12px] font-semibold text-[#182026]">{candidate.claimNumber || 'Recovery reference unavailable'}</p><p className="mt-1 text-[10px] leading-4 text-[#66737F]">Candidate match: {String(candidate.matchType || 'detection').replace(/_/g, ' ')}{candidate.confidence != null ? ` · ${(candidate.confidence * 100).toFixed(0)}% matching signal` : ''}</p></div><Button asChild variant="outline" size="sm" className="h-8 shrink-0 border-[#DCE8EE] bg-white px-2 text-[10px] text-[#4D5B66]"><Link to={tenantRoute(activeSlug, `/recoveries/${candidate.claimId}`)}>Open candidate<ArrowRight className="ml-1 h-3 w-3" /></Link></Button></div>)}</div></div> : null}</section>

                  <section className="grid gap-3 sm:grid-cols-2"><div className="rounded-[8px] border border-[#DCE8EE] p-4"><p className="text-[11px] font-medium text-[#66737F]">What this can support</p><p className="mt-2 text-[12px] leading-5 text-[#4D5B66]">A parsed artifact or recorded relationship can provide source context for case-level evidence review. {linkedCount > 0 ? 'The linked recovery record decides how, if at all, it is evaluated.' : candidateRecoveries.length > 0 ? 'Candidate matches are available for review, but no recovery relationship has been recorded yet.' : 'No recovery relationship has been recorded yet.'}</p></div><div className="rounded-[8px] border border-[#DCE8EE] p-4"><p className="text-[11px] font-medium text-[#66737F]">What this does not establish</p><p className="mt-2 text-[12px] leading-5 text-[#4D5B66]">It does not establish proof, filing authorization, reimbursement eligibility, payment, financial closure, or a recovery outcome by itself.</p></div></section>
