...
const auditDialogContentClass = 'border-[#E8E7E1] bg-white p-0 text-[#191B20] shadow-[0_1px_2px_rgba(25,27,32,0.05)] [&>button]:right-5 [&>button]:top-5 [&>button]:rounded-full [&>button]:p-1 [&>button]:text-[#777A82]';
const auditDialogHeaderClass = 'border-b border-[#E8E7E1] px-5 pb-5 pt-6 pr-14 text-left sm:px-8';
const auditDialogTitleClass = 'font-lora mt-2 text-[26px] font-normal leading-tight tracking-[-0.02em] text-[#191B20]';
const auditDialogDescriptionClass = 'mt-2 max-w-xl text-[13px] leading-5 text-[#595E68]';
const auditDialogBodyClass = 'px-5 py-5 sm:px-8 sm:py-6';
const auditDialogListClass = 'divide-y divide-[#E8E7E1] px-5 py-2 sm:px-8';
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#FBFAF7] font-sans text-[#191B20] selection:bg-[#E9ECFF] selection:text-[#191B20]">
        ...
        <div className="mx-auto flex min-h-14 max-w-[1280px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="hidden min-w-0 border-l border-[#E8E7E1] pl-3 sm:block">
              <p className="text-[10px] font-semibold uppercase tracking-normal text-[#777A82]">Audit workspace</p>
          ...
          <section className="min-w-0 rounded-[14px] border border-[#E8E7E1] bg-white p-5 shadow-[0_1px_2px_rgba(25,27,32,0.05)] sm:p-8" aria-labelledby="audit-workspace-title">
            ...
            {isAuthenticated ? (
              <dl className="grid border-b border-[#E8E7E1] text-left sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="border-b border-[#E8E7E1] py-3.5 sm:pl-4 lg:border-b-0 lg:border-r lg:pr-4"><dt className="text-[10px] font-semibold uppercase tracking-normal text-[#777A82]">Audit record</dt><dd className="mt-1 text-[13px] font-medium text-[#191B20]">{audit ? (selectedAuditIsLatest ? 'Latest audit' : 'From audit history') : 'No audit yet'}</dd></div>
                <div className="border-b border-[#E8E7E1] py-3.5 sm:border-b-0 sm:border-r sm:pr-4 lg:pl-4"><dt className="text-[10px] font-semibold uppercase tracking-normal text-[#777A82]">Source</dt><dd className="mt-1 text-[13px] font-medium text-[#191B20]">{audit ? selectedAuditSource : 'Not recorded'}</dd></div>
                <div className="py-3.5 sm:pl-4"><dt className="text-[10px] font-semibold uppercase tracking-normal text-[#777A82]">Recorded run</dt><dd className="mt-1 text-[13px] font-medium text-[#191B20]">{audit ? selectedAuditRunDate : 'Not recorded'}</dd></div>
              </dl>
              ...
              <SheetContent side="left" className="w-[min(360px,calc(100vw-24px))] border-transparent bg-transparent p-3 text-[#191B20] shadow-none backdrop-blur-0 sm:w-[360px] [&>button]:right-5 [&>button]:top-5 [&>button]:z-10 [&>button]:rounded-full [&>button]:p-1 [&>button]:opacity-60">
                <div className="flex h-full flex-col rounded-[12px] border border-[#D7D7D1] bg-white font-sans shadow-[0_8px_24px_rgba(25,27,32,0.08)]">
                  <SheetHeader className="border-b border-[#E8E7E1] px-4 pb-3 pt-4 pr-12 text-left">
                    ...
                    <SheetDescription className="mt-1.5 text-[12px] leading-4.5 text-[#595E68]">Review the audit record and workspace controls.</SheetDescription>
                  </SheetHeader>
                  <div className="flex-1 overflow-y-auto px-4 py-1.5">
                    <button type="button" onClick={() => { setIsAuditControlsOpen(false); setIsScopeDialogOpen(true); trackEvent('audit_scope_opened', { source_page: '/audit', audit_id: audit?.id || null }); }} className="flex min-h-12 w-full items-center gap-2.5 border-b border-[#E8E7E1] py-2.5 text-left outline-none transition-colors hover:text-[#3F51A8] focus-visible:ring-2 focus-visible:ring-[#5165C7]">
                      <Database className="h-3.5 w-3.5 shrink-0 text-[#595E68]" aria-hidden="true" />
                      <span className="min-w-0 flex-1"><span className="block text-[13px] font-semibold text-[#191B20]">View audit scope</span><span className="mt-0.5 block text-[11px] leading-4.5 text-[#777A82]">See what Margin examined</span></span>
                      <ArrowRight className="h-3 w-3 text-[#777A82]" aria-hidden="true" />
                    </button>
                    <Link to={auditReportUploadHref} onClick={() => setIsAuditControlsOpen(false)} className="flex min-h-12 w-full items-center gap-2.5 border-b border-[#E8E7E1] py-2.5 text-left outline-none transition-colors hover:text-[#3F51A8] focus-visible:ring-2 focus-visible:ring-[#5165C7]">
                      <FilePlus2 className="h-3.5 w-3.5 shrink-0 text-[#595E68]" aria-hidden="true" />
                      <span className="min-w-0 flex-1"><span className="block text-[13px] font-semibold text-[#191B20]">Use Amazon reports</span><span className="mt-0.5 block text-[11px] leading-4.5 text-[#777A82]">Upload reports instead</span></span>
                      <ArrowRight className="h-3 w-3 text-[#777A82]" aria-hidden="true" />
                    </Link>
                    <button type="button" onClick={() => { setIsAuditControlsOpen(false); setIsPeriodSelectorOpen(true); trackEvent('audit_period_selector_opened', { source_page: '/audit' }); }} className="flex min-h-12 w-full items-center gap-2.5 border-b border-[#E8E7E1] py-2.5 text-left outline-none transition-colors hover:text-[#3F51A8] focus-visible:ring-2 focus-visible:ring-[#5165C7]">
                      <Calendar className="h-3.5 w-3.5 shrink-0 text-[#595E68]" aria-hidden="true" />
                      <span className="min-w-0 flex-1"><span className="block text-[13px] font-semibold text-[#191B20]">Audit history</span><span className="mt-0.5 block text-[11px] leading-4.5 text-[#777A82]">Select a recorded audit</span></span>
                      <ArrowRight className="h-3 w-3 text-[#777A82]" aria-hidden="true" />
                    </button>
                    <button type="button" onClick={() => { setIsAuditControlsOpen(false); openAuditLog(); }} className="flex min-h-12 w-full items-center gap-2.5 border-b border-[#E8E7E1] py-2.5 text-left outline-none transition-colors hover:text-[#3F51A8] focus-visible:ring-2 focus-visible:ring-[#5165C7]">
                      <TerminalSquare className="h-3.5 w-3.5 shrink-0 text-[#595E68]" aria-hidden="true" />
                      <span className="min-w-0 flex-1"><span className="block text-[13px] font-semibold text-[#191B20]">Audit activity</span><span className="mt-0.5 block text-[11px] leading-4.5 text-[#777A82]">Review the lifecycle record</span></span>
                      <ArrowRight className="h-3 w-3 text-[#777A82]" aria-hidden="true" />
                    </button>
                    <button type="button" onClick={() => { setIsAuditControlsOpen(false); openScheduleDialog(); }} className="flex min-h-12 w-full items-center gap-2.5 border-b border-[#E8E7E1] py-2.5 text-left outline-none transition-colors hover:text-[#3F51A8] focus-visible:ring-2 focus-visible:ring-[#5165C7]">
                      <CalendarClock className="h-3.5 w-3.5 shrink-0 text-[#595E68]" aria-hidden="true" />
                      <span className="min-w-0 flex-1"><span className="block text-[13px] font-semibold text-[#191B20]">Schedules</span><span className="mt-0.5 block text-[11px] leading-4.5 text-[#777A82]">{weeklyAuditEnabled ? 'Review active preference' : 'Set a workspace preference'}</span></span>
                      <ArrowRight className="h-3 w-3 text-[#777A82]" aria-hidden="true" />
                    </button>
                    <button type="button" onClick={() => { setIsAuditControlsOpen(false); openShareDialog(); }} className="flex min-h-12 w-full items-center gap-2.5 py-2.5 text-left outline-none transition-colors hover:text-[#3F51A8] focus-visible:ring-2 focus-visible:ring-[#5165C7]">
                      <HeartHandshake className="h-3.5 w-3.5 shrink-0 text-[#595E68]" aria-hidden="true" />
                      <span className="min-w-0 flex-1"><span className="block text-[13px] font-semibold text-[#191B20]">Invite a seller</span><span className="mt-0.5 block text-[11px] leading-4.5 text-[#777A82]">Share the audit path</span></span>
                      <ArrowRight className="h-3 w-3 text-[#777A82]" aria-hidden="true" />
                    </button>
                  </div>
                  <div className="border-t border-[#E8E7E1] px-4 py-3"><p className="text-[11px] leading-4.5 text-[#777A82]">Audit records identify potential scope from available data. Filing, evidence use, and recovery action remain seller-controlled and evidence-dependent.</p></div>
                </div>
              </SheetContent>
            </Sheet>
          </aside>
      ...
      <Dialog open={isScopeDialogOpen} onOpenChange={setIsScopeDialogOpen}>
        <DialogContent className={`${auditDialogContentClass} max-h-[min(760px,calc(100vh-32px))] overflow-y-auto rounded-[14px] sm:max-w-[660px]`}>
          <DialogHeader className={auditDialogHeaderClass}>
            <p className="text-[11px] font-semibold uppercase tracking-normal text-[#777A82]">Selected audit record</p>
            <DialogTitle className={auditDialogTitleClass}>What Margin examined</DialogTitle>
            <DialogDescription className={auditDialogDescriptionClass}>This records the known scope and limits of the selected audit. It does not turn a finding, estimate, or available data into proof, filing authority, reimbursement eligibility, payment, or closure.</DialogDescription>
          </DialogHeader>
          <div className={auditDialogBodyClass}>
            <dl className="grid border-y border-[#E8E7E1] sm:grid-cols-2">
              <div className="border-b border-[#E8E7E1] py-3 sm:border-r sm:pr-4"><dt className="text-[10px] font-semibold uppercase tracking-normal text-[#777A82]">Workspace</dt><dd className="mt-1 text-[13px] font-medium text-[#191B20]">{activeWorkspaceLabel}</dd></div>
              <div className="border-b border-[#E8E7E1] py-3 sm:pl-4"><dt className="text-[10px] font-semibold uppercase tracking-normal text-[#777A82]">Audit selected</dt><dd className="mt-1 text-[13px] font-medium text-[#191B20]">{audit ? (selectedAuditIsLatest ? 'Latest audit' : 'Selected audit from history') : 'No audit selected'}</dd></div>
              <div className="border-b border-[#E8E7E1] py-3 sm:border-b-0 sm:border-r sm:pr-4"><dt className="text-[10px] font-semibold uppercase tracking-normal text-[#777A82]">Source</dt><dd className="mt-1 text-[13px] font-medium text-[#191B20]">{audit ? selectedAuditSource : 'Not recorded'}</dd></div>
      ...
      <Dialog open={isPeriodSelectorOpen} onOpenChange={setIsPeriodSelectorOpen}>
        <DialogContent className={`${auditDialogContentClass} max-h-[min(760px,calc(100vh-32px))] overflow-y-auto rounded-[14px] sm:max-w-[650px]`}>
          <DialogHeader className={auditDialogHeaderClass}>
            <p className="text-[11px] font-semibold uppercase tracking-normal text-[#777A82]">Audit records</p>
            <DialogTitle className={auditDialogTitleClass}>Audit history</DialogTitle>
            <DialogDescription className={auditDialogDescriptionClass}>Select an actual audit record from this workspace. Run date, source, and coverage are shown only when recorded; a detailed audit period may not be available for older audits.</DialogDescription>
          </DialogHeader>
          <div className={auditDialogBodyClass}>
      ...
      <Dialog open={isExportDialogOpen} onOpenChange={setIsExportDialogOpen}>
        <DialogContent className={`${auditDialogContentClass} rounded-[14px] sm:max-w-[540px]`}>
          <DialogHeader className={auditDialogHeaderClass}><p className="text-[11px] font-semibold uppercase tracking-normal text-[#777A82]">Selected audit record</p><DialogTitle className={auditDialogTitleClass}>Export summary</DialogTitle><DialogDescription className={auditDialogDescriptionClass}>Download a browser-generated record for {selectedAuditSelectorLabel}. It is intended for management or operational review; sensitive identifiers, tokens, raw payloads, and payment references are excluded.</DialogDescription></DialogHeader>
          <div className={auditDialogBodyClass}>
      ...
      <Dialog open={isScheduleDialogOpen} onOpenChange={setIsScheduleDialogOpen}>
        <DialogContent className={`${auditDialogContentClass} max-h-[min(780px,calc(100vh-32px))] overflow-y-auto rounded-[14px] sm:max-w-[680px]`}>
          <DialogHeader className={auditDialogHeaderClass}><p className="text-[11px] font-semibold uppercase tracking-normal text-[#777A82]">Workspace run preference</p><DialogTitle className={auditDialogTitleClass}>Automatic audit schedule</DialogTitle><DialogDescription className={auditDialogDescriptionClass}>This is a workspace-owned run preference. It does not create proof, filing authority, reimbursement eligibility, payment, or case closure.</DialogDescription></DialogHeader>
          <div className={auditDialogBodyClass}>
