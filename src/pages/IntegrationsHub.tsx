        ...
        <div className="relative w-full bg-gray-50 min-h-[calc(100vh+96px)] -mt-24 pt-24">
          ...
          {/* Header */}
            <div className="text-center">
              <h1 className="text-3xl font-bold mb-2 text-black">Integrations</h1>
            ...
            <Card className="bg-white border-gray-200 text-gray-900">
              <CardHeader className="space-y-4">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  ...
                  <Badge variant="outline" className={cn('text-xs px-3 py-1 rounded-full', (isSandbox || status?.amazon_connected) ? 'border-emerald-500 text-emerald-600 font-semibold' : 'border-gray-300 text-gray-600')}>
              ...
              <CardContent className="space-y-6">
                <div className="grid gap-3 text-sm text-gray-900">
                  <div className="flex items-center justify-between text-xs uppercase tracking-wide text-gray-600">
                    <span>Status</span>
                    <span className={cn('font-semibold', (isSandbox || status?.amazon_connected) ? 'text-emerald-600' : 'text-amber-600')}>
                      {(isSandbox || status?.amazon_connected) ? 'Active' : 'Awaiting Connection'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Last sync</span>
                    <span className="text-black">{status?.lastSync || lastSyncTime}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs text-gray-600">
                    <span>Scopes</span>
                    <span className="text-black text-right">orders.read · inventory.read · transactions.read</span>
                  </div>
                </div>
                <Button 
                  size="default" 
          ...
          <Card className="bg-white border-gray-200 text-gray-900">
            <CardHeader>
                <CardTitle className="text-black">Document Source</CardTitle>
              ...
              {evidenceSources.length > 0 && (
                <div className="mb-4 p-3 rounded border border-gray-200 bg-gray-50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {evidenceSources.filter(s => s.status === 'connected').map((source) => (
                      <Badge key={source.id} variant="outline" className="border-emerald-500 text-emerald-600 bg-emerald-50">
                      ...
                      return (
                        <div key={p} className="flex flex-col gap-4 px-4 py-4 md:flex-row md:items-center md:justify-between">
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-black">Gmail</p>
                          ...
                          <div className="flex flex-col gap-2 md:flex-row md:items-center">
                            <Badge variant="outline" className={cn('w-fit text-xs', connected ? 'border-emerald-500 text-emerald-600 font-semibold' : hasError() ? 'border-red-400 text-red-600' : 'border-gray-300 text-gray-600')}>
                              {connected ? 'Connected' : hasError() ? 'Error' : 'Not connected'}
                            </Badge>
                              <div className="flex gap-2">
                                <Button 
                                  size="sm" 
                                  className={cn(connected ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100' : 'bg-emerald-500 hover:bg-emerald-600 text-white')} 
                ...
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 space-y-5">
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-sm font-semibold text-black">Auto‑collect</p>
                      <p className="text-xs text-gray-600">Automatically sweep connected inboxes for invoices and receipts.</p>
                    </div>
                    <Button 
                      size="sm" 
                      variant="outline" 
                  ...
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-sm font-semibold text-black">Schedule</p>
                      <p className="text-xs text-gray-600">Choose how often evidence is ingested.</p>
                    </div>
                    <Button 
                      size="sm" 
                      variant="outline" 
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="text-sm font-semibold text-black">Last ingest</p>
                      <p className="text-xs text-gray-600">{status?.lastIngest || 'Just now'}</p>
                    </div>
                    <Button 
                      size="lg" 
                    ...
                    {ingestionResult && (
                      <div className={`rounded-lg border p-4 ${ingestionResult.success ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50'}`}>
                        <div className="flex items-center justify-between mb-2">
                          <h4 className={`font-semibold ${ingestionResult.success ? 'text-emerald-700' : 'text-red-700'}`}>
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            {ingestionResult.results && (
                              <div className="mt-3 space-y-1">
                                <p className="text-xs font-semibold text-gray-600 uppercase">Breakdown by Provider:</p>
                                {Object.entries(ingestionResult.results).map(([provider, result]) => (
                                  <div key={provider} className="flex items-center justify-between text-xs">
                                    <span className="text-gray-700 capitalize">{provider === 'gdrive' ? 'Google Drive' : provider}:</span>
                    ...
                    <div className="flex flex-col gap-2 pt-2 md:flex-row md:items-center md:justify-between">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => navigate('/evidence-locker')} className="text-gray-600 hover:text-gray-800">
