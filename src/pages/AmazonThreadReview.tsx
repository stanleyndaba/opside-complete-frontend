  ...
  return <main className="min-h-screen bg-[#FAFAF7] font-sans text-[#182026]">
    ...
    <div className="mx-auto max-w-[1280px] px-4 py-4 sm:px-6 sm:py-6">
      <section className="rounded-[10px] border border-[#DCE8EE] bg-white p-4 shadow-[0_1px_2px_rgba(24,32,38,0.03)] sm:p-5">
        <div className="mb-4 border-b border-[#E7EEF2] pb-3"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">Amazon records</p><h1 className="mt-0.5 font-lora text-[16px] font-normal tracking-tight text-[#182026] sm:text-[19px]">What Amazon said and what Margin sent</h1><p className="mt-1 text-[11px] leading-5 text-[#66737F]">Inspect Amazon&apos;s case state, messages, attachments, and the evidence-backed reply from this recovery record.</p></div>
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-3"><h2 className="text-sm font-semibold tracking-tight text-[#07111A]">Amazon Thread</h2><span className="rounded-full bg-[#F1F3F4] px-2.5 py-1 text-[10px] uppercase tracking-tight text-[#36404A]">Reimbursement recorded in thread</span></div>
          ...
          <div className="space-y-3">{messages.map((message) => <article key={message.subject} className={`space-y-3 rounded-[5px] border px-4 py-4 ${message.direction === "inbound" ? "border-blue-500/20 bg-blue-500/[0.05]" : "border-emerald-500/20 bg-emerald-500/[0.05]"}`}>
            <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-tight"><span className="rounded-full bg-[#F1F3F4] px-2.5 py-1 text-[#36404A]">{message.direction === "inbound" ? "Inbound" : "Outbound"}</span><span className="text-[#6B7C88]">{message.date}</span><span className="rounded-full bg-[#F1F3F4] px-2.5 py-1 text-[#36404A]">{message.state}</span></div>
