      ...
      <div className="relative z-10 flex w-full items-center">
        <div className="max-w-[1040px]">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.78, ease: [0.22, 1, 0.36, 1] }} className="mt-6 flex w-full flex-col items-stretch gap-3 sm:mt-10 sm:w-auto sm:flex-row sm:items-center sm:gap-6">
            ...
            <button type="button" onClick={onReportCta} className="group relative inline-flex h-[50px] w-full items-center justify-center overflow-hidden px-1 text-[16px] font-semibold text-slate-200 transition-colors duration-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B0D12] sm:h-11 sm:w-auto sm:justify-start sm:text-[15px]" aria-label="Use Amazon Reports"><span className="relative z-10 inline-block leading-[1.25]">Use Amazon Reports{!reduceMotion ? <motion.span aria-hidden="true" className="pointer-events-none absolute inset-0 whitespace-nowrap bg-[linear-gradient(110deg,#CBD5E1_28%,#FFFFFF_47%,#CBD5E1_66%)] bg-[length:260%_100%] bg-clip-text text-transparent" animate={{ backgroundPosition: ["-175% 0%", "175% 0%"], opacity: [0, 0.9, 0] }} transition={{ duration: 1.7, delay: 1.45, repeat: Infinity, repeatDelay: 3.4, ease: [0.23, 1, 0.32, 1] }}>Use Amazon Reports</motion.span> : null}</span></button>
          </motion.div>
