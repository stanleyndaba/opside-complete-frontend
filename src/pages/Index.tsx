          ...
          <motion.div {...revealProps} className="relative min-w-0 overflow-hidden rounded-[12px] border border-[#373735] bg-[#1B1B1B] p-5 text-[#E7E5DF] sm:p-8 md:p-10">
            ...
            <div className="relative grid gap-10 md:grid-cols-[1.1fr_0.9fr] md:gap-0">
              <div className="relative md:pr-8">
                ...
                <div className="relative flex items-start gap-4 py-2"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#777A76] text-[#E7E5DF]"><svg aria-hidden="true" viewBox="0 0 36 24" className="h-5 w-7 fill-none stroke-current" strokeWidth="1.35"><circle cx="18" cy="7" r="3.2" /><path d="M11.5 19c.4-3.3 2.8-5.2 6.5-5.2s6.1 1.9 6.5 5.2" /><circle cx="7.5" cy="9" r="2.5" /><path d="M2.5 19c.3-2.6 2.1-4.2 5-4.2 1.5 0 2.8.4 3.7 1.2" /><circle cx="28.5" cy="9" r="2.5" /><path d="M33.5 19c-.3-2.6-2.1-4.2-5-4.2-1.5 0-2.8.4-3.7 1.2" /></svg></div><div className="min-w-0 flex-1 space-y-2.5">{responsibilityCycle.map((step) => { const isActive = step === activeResponsibility; return <div key={step} className="flex items-center gap-3"><div className="relative h-px flex-1 bg-[#777A76]"><motion.span aria-hidden="true" className="absolute -top-[3px] h-[7px] w-[7px] rounded-full bg-[#E45B2A] shadow-[0_0_0_3px_rgba(228,91,42,0.12)]" initial={false} animate={isActive && !reduceMotion ? { left: ["0%", "100%", "0%"] } : { left: isActive ? "100%" : "0%", opacity: isActive ? 1 : 0 }} transition={isActive && !reduceMotion ? { duration: 1.8, ease: "easeInOut", repeat: Infinity, repeatDelay: 0.2 } : { duration: 0.2 }} /></div><AnimatePresence mode="wait" initial={false}><span className={`w-[82px] shrink-0 text-[12px] font-medium leading-4 sm:w-[96px] sm:text-[13px] ${isActive ? "text-[#E7E5DF]" : "text-[#A8AAA5]"}`}>{step}</span></AnimatePresence></div>; })}</div></div>
