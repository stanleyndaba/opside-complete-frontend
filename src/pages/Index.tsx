    ...
    <section ref={sectionRef} aria-labelledby="operational-economics-title" className="relative overflow-hidden border-b border-[var(--margin-border)] bg-[var(--margin-canvas)] py-16 sm:py-20 md:py-28">
      <div className={containerClass}>
        <div className="grid items-start gap-12 lg:grid-cols-[1.12fr_0.88fr] lg:gap-20">
          <motion.div {...revealProps} className="relative min-w-0 overflow-hidden rounded-[12px] border border-[var(--margin-border)] bg-[#F4F4F0] p-5 sm:p-8 md:p-10">
            <div className="mb-9 flex items-center justify-between gap-4"><span className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--margin-text-muted)]">The work keeps coming back</span><span className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--margin-text-muted)]">Every month</span></div>
            <div className="relative grid gap-10 md:grid-cols-[1.1fr_0.9fr] md:gap-0">
              <div className="relative md:pr-8">
                <div className="mb-4 flex items-center justify-between"><span className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--margin-text-muted)]">Your team</span><span className="font-mono text-[9px] uppercase tracking-[0.12em] text-[var(--margin-text-muted)]">Owns the function</span></div>
                <div className="pointer-events-none absolute -left-2 right-5 top-[54px] space-y-[31px] opacity-20" aria-hidden="true">{selfPath.map((step) => <div key={`ghost-${step}`} className="h-px bg-[#8E9590]" />)}</div>
                <div className="relative space-y-4">{selfPath.map((step, index) => <div key={step} className="relative flex items-center gap-3"><span className="relative z-10 h-2 w-2 shrink-0 rounded-full bg-[#7C8580]" /><div className="relative h-px flex-1 bg-[#AEB4B0]"><motion.span aria-hidden="true" className="absolute -top-[3px] h-[7px] w-[7px] rounded-full bg-[#68716C]" initial={{ left: "0%", opacity: 0 }} animate={reduceMotion || !sectionInView ? { left: "0%", opacity: 0 } : { left: ["0%", "100%"], opacity: [0, 1, 1, 0] }} transition={{ duration: 3.8, delay: index * 0.16, ease: "easeInOut", repeat: reduceMotion ? 0 : Infinity, repeatDelay: 1.4 }} /></div><span className="w-[68px] shrink-0 text-[12px] font-medium leading-4 text-[var(--margin-text-secondary)] sm:w-[82px] sm:text-[13px]">{step}</span></div>)}</div>
                <p className="mt-7 border-t border-dashed border-[#C5C9C5] pt-4 text-center font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-[var(--margin-text-muted)]">Six responsibilities · one recurring burden</p>
              </div>
              <div className="relative border-t border-[var(--margin-border)] pt-8 md:ml-0 md:border-l md:border-t-0 md:pl-8 md:pt-0">
                <div className="mb-4"><span className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--margin-blue)]">With Margin</span><span className="mt-1 block font-mono text-[9px] uppercase tracking-[0.12em] text-[var(--margin-text-muted)]">One operating layer</span></div>
                <div className="relative mt-8 h-[112px] rounded-[8px] border border-[var(--margin-blue)]/20 bg-white/70 p-4"><div className="absolute left-4 right-4 top-1/2 h-[2px] -translate-y-1/2 bg-[var(--margin-blue)]" /><motion.span aria-hidden="true" className="absolute top-1/2 -ml-1.5 h-3 w-3 -translate-y-1/2 rounded-full bg-[var(--margin-blue)] shadow-[0_0_0_5px_rgba(11,116,222,0.12)]" initial={{ left: "16%" }} animate={reduceMotion || !sectionInView ? { left: "16%" } : { left: ["16%", "84%"] }} transition={{ duration: 5.2, ease: "easeInOut", repeat: reduceMotion ? 0 : Infinity, repeatDelay: 1.2 }} /><div className="absolute inset-x-4 bottom-3 flex justify-between gap-2 text-[10px] font-medium leading-3 text-[var(--margin-text-primary)]"><span>Amazon<br />activity</span><span className="text-center">Recovery<br />record</span><span className="text-right">Verified<br />outcome</span></div></div>
                <p className="mt-6 border-t border-[var(--margin-blue)]/20 pt-4 font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-[var(--margin-blue)]">The burden becomes one connected recovery record.</p>
              </div>
            </div>
            <div className="pointer-events-none absolute left-[57%] top-[46px] hidden h-[calc(100%-92px)] w-px bg-[var(--margin-blue)]/30 md:block" aria-hidden="true" /><div className="mt-9 border-t border-[var(--margin-border)] pt-4 text-center font-lora text-[18px] leading-tight tracking-[-0.02em] text-[var(--margin-text-primary)] sm:text-[22px]" style={{ fontWeight: 400 }}>The work is transferred. The authority stays with you.</div>
          </motion.div>
          ...
          <motion.div {...revealProps} transition={{ ...revealProps.transition, delay: 0.1 }}>
            ...
            <div className="mt-8 border-t border-[var(--margin-border)]"><div className="border-b border-[var(--margin-border)] py-4"><p className="text-[14px] font-semibold text-[var(--margin-text-primary)]">Recurring</p><p className="mt-1 text-[13px] leading-5 text-[var(--margin-text-secondary)]">The work returns whenever the next issue appears.</p></div><div className="border-b border-[var(--margin-border)] py-4"><p className="text-[14px] font-semibold text-[var(--margin-text-primary)]">Connected</p><p className="mt-1 text-[13px] leading-5 text-[var(--margin-text-secondary)]">Evidence, cases, responses, payouts, and outcomes stay together.</p></div><div className="border-b border-[var(--margin-border)] py-4"><p className="text-[14px] font-semibold text-[var(--margin-text-primary)]">Owned by Margin</p><p className="mt-1 text-[13px] leading-5 text-[var(--margin-text-secondary)]">Your team remains the authority without carrying the entire operating burden.</p></div></div>
