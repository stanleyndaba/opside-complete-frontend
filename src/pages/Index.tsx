      ...
      <div className="relative z-10 flex w-full items-center">
        <div className="max-w-[1040px]">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.78, ease: [0.22, 1, 0.36, 1] }} className="mt-6 flex w-full flex-col items-stretch gap-3 sm:mt-10 sm:w-auto sm:flex-row sm:items-center sm:gap-6">
            <Button onClick={onAuditCta} aria-label="Connect my Amazon account" className="landing-pressable group relative h-[54px] w-full justify-center overflow-hidden rounded-[8px] bg-[var(--margin-blue)] px-6 text-[15px] font-bold text-white shadow-[0_18px_48px_rgba(23,92,211,0.34)] transition-[background-color,box-shadow] duration-200 hover:bg-[var(--margin-blue-hover)] sm:h-[56px] sm:w-auto sm:px-10 sm:text-[16px]"><div className="absolute inset-0 bg-white/20 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />Connect my Amazon account <ArrowRight className="ml-2 h-5 w-5" /></Button>
            <button type="button" onClick={onReportCta} className="group inline-flex h-[50px] w-full items-center justify-center px-1 text-[16px] font-semibold text-[#98A2B3] transition-colors duration-200 hover:text-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B0D12] sm:h-11 sm:w-auto sm:justify-start sm:text-[15px]" aria-label="Upload Amazon reports instead">{reduceMotion ? <span className="leading-[1.25]">Upload Amazon reports instead</span> : <motion.span className="inline-block leading-[1.25] bg-[linear-gradient(110deg,#98A2B3_0%,#98A2B3_39%,#F8FAFC_50%,#BFC7D2_61%,#98A2B3_100%)] bg-[length:240%_100%] bg-clip-text text-transparent" initial={{ backgroundPosition: "145% 0%" }} animate={{ backgroundPosition: ["145% 0%", "-145% 0%"] }} transition={{ duration: 1.9, delay: 1, repeat: Infinity, repeatDelay: 2.4, ease: [0.23, 1, 0.32, 1] }}>Upload Amazon reports instead</motion.span>}</button>
          </motion.div>
