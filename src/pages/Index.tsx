          ...
          <motion.div {...revealProps} className="relative min-w-0 overflow-hidden rounded-[12px] border border-[#373735] bg-[#1B1B1B] p-5 text-[#E7E5DF] sm:p-8 md:p-10">
            ...
            <div className="relative grid gap-10 md:grid-cols-[1.1fr_0.9fr] md:gap-0">
              <div className="relative md:pr-8">
                ...
                <div className="relative flex items-center gap-4 py-2"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#777A76] text-[#E7E5DF]"><svg aria-hidden="true" viewBox="0 0 36 24" className="h-5 w-7 fill-none stroke-current" strokeWidth="1.35"><circle cx="18" cy="7" r="3.2" /><path d="M11.5 19c.4-3.3 2.8-5.2 6.5-5.2s6.1 1.9 6.5 5.2" /><circle cx="7.5" cy="9" r="2.5" /><path d="M2.5 19c.3-2.6 2.1-4.2 5-4.2 1.5 0 2.8.4 3.7 1.2" /><circle cx="28.5" cy="9" r="2.5" /><path d="M33.5 19c-.3-2.6-2.1-4.2-5-4.2-1.5 0-2.8.4-3.7 1.2" /></svg></div><div className="h-px flex-1 bg-[#A8AAA5]" /><AnimatePresence mode="wait" initial={false}><motion.span key={activeResponsibility} initial={reduceMotion ? { opacity: 1 } : { opacity: 0, filter: "blur(8px)", y: 5 }} animate={{ opacity: 1, filter: "blur(0px)", y: 0 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, filter: "blur(8px)", y: -5 }} transition={{ duration: reduceMotion ? 0 : 1, ease: [0.22, 1, 0.36, 1] }} className="w-[92px] shrink-0 text-[17px] font-medium leading-5 text-[#E7E5DF] sm:w-[108px] sm:text-[19px]">{activeResponsibility}</motion.span></AnimatePresence></div>
              ...
              <div className="relative border-t border-[#3D3E3B] pt-6 md:ml-0 md:border-l md:border-t-0 md:pl-8 md:pt-0">
                ...
                <div className="relative mt-6 h-[116px] overflow-hidden rounded-[8px] border border-[#555653] bg-[#D1D0C8] p-2.5"><AnimatePresence mode="wait" initial={false}><motion.div key={activeNotification.title} initial={reduceMotion ? { opacity: 1, y: 0, filter: "blur(0px)" } : { opacity: 0, y: 22, filter: "blur(5px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: -22, filter: "blur(5px)" }} transition={{ duration: reduceMotion ? 0 : 0.75, ease: [0.22, 1, 0.36, 1] }} className="flex h-full items-center gap-3 rounded-[6px] border border-[#B5B5AD] bg-[#E7E5DF] px-3.5 py-3 text-[#343532] shadow-[0_2px_8px_rgba(52,53,50,0.08)]"><span className="h-2 w-2 shrink-0 rounded-full bg-[#343532]" /><div className="min-w-0 flex-1"><p className="font-mono text-[9px] font-semibold uppercase tracking-[0.1em] text-[#6E706B]">Live recovery movement</p><p className="mt-1 text-[15px] font-semibold leading-5">{activeNotification.title}</p><p className="mt-0.5 text-[13px] leading-4">{activeNotification.detail}</p></div><span className="hidden max-w-[88px] text-right text-[9px] leading-3 text-[#6E706B] sm:block">{activeNotification.meta}</span></motion.div></AnimatePresence></div>
