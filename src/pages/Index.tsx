      ...
      <div className="relative flex h-full items-center justify-center p-3 sm:p-5">
        <div className="w-full max-w-[390px] space-y-2.5">
          {items.map(({ title, detail, status, tone }) => (
            <div key={title} className="rounded-[10px] bg-[#F3F0E8]/90 px-3 py-2.5 shadow-[0_14px_30px_rgba(20,31,34,0.18)] backdrop-blur-xl sm:px-4 sm:py-3">
  ...
  function FulfillmentReportVisual() {
    return (
      <div className="relative mb-6 aspect-[4/3] overflow-hidden rounded-[4px] border border-[#BFC5C1] bg-[#D7D9D5]" aria-label="Fulfillment and settlement reconciliation report">
        ...
        <div className="absolute inset-x-2 bottom-2 min-h-[238px] rounded-[10px] bg-white/75 p-3 shadow-[0_14px_30px_rgba(20,31,34,0.18)] backdrop-blur-xl sm:inset-x-3 sm:bottom-3 sm:min-h-[250px] sm:p-4">
          <div className="mb-2 text-[10px] font-semibold tracking-tight text-[#52616A]">Evidence context</div>
          <div className="relative pl-7">
            <span className="absolute left-[9px] top-3 bottom-3 w-px bg-[#C7D0CC]" aria-hidden="true" />
            {[
              ['Record', 'bg-[#4FB879]'],
              ['Movement', 'bg-[#4FB879]'],
              ['Event', 'bg-[#C99A42]'],
              ['Settlement', 'bg-[#4FB879]'],
            ].map(([label, color], index) => (
              <div key={label} className="relative flex items-center gap-2 py-1">
                <span className={`absolute -left-7 flex h-[18px] w-[18px] items-center justify-center rounded-full ${color} text-white`}><Check className="h-2.5 w-2.5" strokeWidth={3} /></span>
                <span className="text-[10px] font-semibold tracking-tight text-[#263438]">{label}</span>
                {index === 2 ? <span className="ml-auto text-[8px] font-semibold tracking-tight text-[#9A6A20]">1 unit unresolved</span> : null}
              </div>
            ))}
          </div>
  ...
  return (
    <div className={`relative ${compactMobile ? "max-h-[80vh] overflow-y-auto overscroll-contain touch-pan-y sm:max-h-none sm:overflow-visible" : "overflow-hidden"}`}>
      <div className="mx-auto max-h-none w-full max-w-[900px] overflow-visible rounded-[10px] sm:max-h-[620px] sm:overflow-hidden border border-[#DCE8EE] bg-white text-[#182026] shadow-[0_18px_45px_rgba(24,32,38,0.16)]">
        ...
        {isProof ? <div className="grid gap-0 lg:grid-cols-[1.2fr_0.8fr]"><div className="border-b border-[#E9E9EC] px-4 py-3 lg:border-b-0 lg:border-r"><div className="mb-3 flex gap-2"><span className="rounded-[10px] border-0 bg-[#EEF1F2] px-2 py-1 text-[10px] text-[#182026]">Fee Charge Review</span><span className="rounded-[10px] border-0 bg-[#EEF1F2] px-2 py-1 text-[10px] font-medium text-[#182026]">Claim candidate</span></div><p className="text-[10px] font-medium uppercase tracking-tight text-[#66737F]">Required documentation</p><div className="mt-2 border-y border-[#E9E9EC] py-1">{[['Fee event or settlement row','The charged fee transaction, fee type, settlement ID, order ID, or shipment context.'],['Product and marketplace context','SKU/ASIN, marketplace, fulfilment channel, size tier, dimensions, weight, and category where available.'],['Expected fee basis','The schedule version, rate basis, or calculation inputs Margin is comparing against the charge.'],['Charged-versus-expected amount','The actual fee, expected fee, currency, and overcharge delta for the same event.']].map(([label,copy], index, items) => <div key={label} className="relative flex gap-3 py-3 first:pt-2 last:pb-2"><div className="relative flex w-3 shrink-0 justify-center"><span className="relative z-10 mt-1.5 h-2.5 w-2.5 rounded-full bg-[#0B74DE] ring-4 ring-white" aria-hidden="true" />{index < items.length - 1 && <span className="absolute left-1/2 top-4 h-[calc(100%+1px)] w-px -translate-x-1/2 bg-[#C9D6DE]" aria-hidden="true" />}</div><div className="min-w-0"><p className="text-[11px] font-medium tracking-tight text-[#4D5B66]">{label}</p><p className="mt-1 text-[10px] leading-4 text-[#66737F]">{copy}</p></div></div>)}</div><div className="mt-3 border-t border-[#E9E9EC] pt-3"><p className="text-[10px] font-medium uppercase tracking-tight text-[#66737F]">What Margin already found</p><p className="mt-1 text-[11px] leading-5 text-[#4D5B66]">Order 113-9204451-1885026 · Settlement SETTLE-ACME-003 · SKU ACME-DESK-LAMP-OAK</p><div className="mt-2 grid grid-cols-2 gap-2 text-[10px] text-[#6B7280]"><span>Order ID<br /><strong className="text-[#182026]">113-9204451-1134623</strong></span><span>SKU<br /><strong className="text-[#182026]">ACME-DEMO-SKU-18</strong></span><span>ASIN<br /><strong className="text-[#182026]">B0ACME0018</strong></span><span>Shipment<br /><strong className="text-[#182026]">FBA17ACME018</strong></span></div></div><div className="mt-3 border-t border-[#E9E9EC] pt-3"><p className="text-[10px] font-medium uppercase tracking-tight text-[#66737F]">If proof is missing</p><p className="mt-1 text-[11px] leading-5 text-[#4D5B66]">Margin keeps looking across connected repositories before asking the seller. If the required document is not found, upload it in Evidence Records and Margin can attach it to the filing workflow.</p></div></div><div className="px-4 py-3"><p className="text-[10px] font-medium uppercase tracking-tight text-[#66737F]">Why this proof matters</p><p className="mt-2 text-[11px] leading-5 text-[#4D5B66]">Margin checks the fee event against the available product, order, shipment, and fee basis before treating it as a supported case.</p><div className="mt-4 space-y-3 border-t border-[#E9E9EC] pt-3"><p className="text-[10px] font-medium uppercase tracking-tight text-[#66737F]">Amazon policy basis</p><p className="text-[11px] leading-5 text-[#4D5B66]">Amazon selling and FBA fee schedule review</p><p className="text-[10px] font-medium uppercase tracking-tight text-[#66737F]">Claim window</p><p className="text-[10px] text-[#4D5B66]">28 days left · Deadline Feb 18, 2026</p></div></div></div> : <div className="grid gap-0 lg:grid-cols-[1.2fr_0.8fr]"><div className="border-b border-[#E9E9EC] px-4 py-3 lg:border-b-0 lg:border-r"><div className="grid border-y border-[#E9E9EC] md:grid-cols-3 md:divide-x md:divide-[#F0F0F2]"><div className="py-2 md:pr-3"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">Amount under review</p><p className="mt-1 font-lora text-[17px] font-normal leading-tight tracking-tight">$900.95</p></div><div className="border-t border-[#E9E9EC] py-2 md:border-t-0 md:px-3"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">Found on</p><p className="mt-1 text-[11px] font-medium">Jan 21, 2026, 04:37 PM</p></div><div className="border-t border-[#E9E9EC] py-2 md:border-t-0 md:pl-3"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">Amazon source / activity</p><p className="mt-1 text-[11px] font-medium tracking-tight">Amazon FBA Settlement Activity</p><span className="mt-2 inline-flex rounded-[10px] border-0 bg-[#EEF1F2] px-2 py-0.5 text-[10px] font-medium text-[#182026]">Evidence-ready</span></div></div><div className="mt-3"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">What Margin found</p><p className="mt-1 text-[11px] leading-5 text-[#4D5B66]">A storage-related charge appears to have been applied more than the seller record supports. · $1484.80 charged vs $742.40 expected</p><span className="mt-2 inline-flex rounded-[10px] border-0 bg-[#EEF1F2] px-2 py-0.5 text-[10px] text-[#182026]">Fee discrepancy</span><div className="mt-3 border-t border-[#E9E9EC] pt-2.5"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">Evidence used</p><p className="mt-1 text-[11px] leading-5 text-[#4D5B66]">Order 113-9204451-1885026 · Settlement SETTLE-ACME-003 · SKU ACME-DESK-LAMP-OAK</p></div><div className="mt-3 border border-[#CFE0EA] bg-[#F7FBFF] p-2.5"><div className="flex items-center justify-between gap-2"><p className="text-[11px] font-medium tracking-tight text-[#0B74DE]">Margin analysis</p><p className="text-[11px] font-medium tracking-tight tracking-tight text-[#26734D]">Investigation complete</p></div><p className="mt-1 text-[11px] leading-5 text-[#4D5B66]">Margin connected the Amazon activity, seller records, and expected outcome to identify the recovery gap.</p><div className="mt-2 grid grid-cols-3 gap-2 border-t border-[#DCEAF4] pt-2 text-[10px]"><span>Charge applied<br /><strong>$1,792.82</strong></span><span>Should have been<br /><strong>$828.87</strong></span><span>Difference<br /><strong className="text-[#0B74DE]">$900.95</strong></span></div></div><div className="mt-3 border-y border-[#E9E9EC] bg-[#FFFBF8] py-2"><p className="text-[11px] font-medium tracking-tight text-[#A95F49]">Why unresolved</p><p className="mt-1 text-[10px] leading-4 text-[#6B7280]">Evidence is assembled; Margin is waiting for the filing gate to clear.</p></div></div></div><div className="px-4 py-3"><p className="text-[11px] font-medium tracking-tight text-[#66737F]">Backend detection record</p><div className="mt-2 grid grid-cols-2 gap-3 border-b border-[#E9E9EC] pb-3 text-[10px]"><span>Backend record<br /><strong>demo-finding</strong></span><span>Source<br /><strong>SP API</strong></span><span>Confidence<br /><strong>94%</strong></span><span>Readiness<br /><strong>Claim candidate</strong></span></div><p className="mt-3 text-[11px] font-medium tracking-tight text-[#66737F]">Amazon policy basis</p><p className="mt-1 text-[11px] leading-5 text-[#4D5B66]">Amazon selling and FBA fee schedule review</p><p className="mt-3 text-[11px] font-medium tracking-tight text-[#66737F]">Deadline</p><p className="mt-1 text-[10px] text-[#4D5B66]">28 days left · Feb 18, 2026</p></div></div>}
  ...
  return (
    <div className="landing-google-sans min-h-screen bg-[var(--margin-canvas)] selection:bg-[var(--margin-blue)]/16 selection:text-[var(--margin-text-primary)]">
      ...
      <main>
        ...
        {/* Solution — What does Margin actually do? */}
        <MarginEngineSection />
