    ...
    <div className="mx-auto max-w-[1600px] px-4 py-4 sm:px-6 sm:py-6">
      <section className="overflow-hidden rounded-[10px] border border-[#DCE8EE] bg-white shadow-[0_2px_8px_rgba(24,32,38,0.03)]">
        <header className="border-b border-[#DCE8EE] px-5 py-5 sm:px-6">
          ...
          <h1 className="mt-1 font-lora text-[28px] font-normal leading-tight tracking-tight text-[#182026] sm:text-[32px]">Appeals review</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1400px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-[#F3F5F4]">{filtered.map((item) => <tr key={item.id} className="group align-top transition-colors hover:bg-[#F3F5F4]/50">
              <td className="px-6 py-6"><div className="w-[280px] space-y-4"><div className="space-y-1.5"><p className="text-[14px] font-bold tracking-tight text-[#111827]">{item.id}</p><p className="text-[12px] font-medium text-[#6B7280]">{item.store}</p><p className="text-[11px] font-medium tracking-tight text-[#9CA3AF]">{item.identifiers}</p><div className="pt-1 flex items-center gap-1.5"><span className="text-[9px] font-bold tracking-tight text-[#D1D5DB]">Case progress:</span><span className={`text-[10px] font-bold tracking-tight ${item.progressTone}`}>{item.progress}</span></div></div><div className="border-t border-[#F3F5F4] pt-4 space-y-2"><p className="text-[10px] font-bold tracking-tight text-[#9CA3AF]">Amount at stake</p><div className="space-y-1 text-[12px]"><div className="flex justify-between"><span className="text-[#9CA3AF]">Requested</span><span className="font-semibold text-[#111827]">{item.requested}</span></div><div className="flex justify-between"><span className="text-[#9CA3AF]">Amazon approval</span><span className="font-semibold text-[#111827]">{item.approved}</span></div><div className="flex justify-between"><span className="text-[#9CA3AF]">Review gap</span><span className="font-bold text-[#111827]">{item.gap}</span></div></div></div></div></td>
              <td className="px-6 py-6"><div className="w-[300px] space-y-4"><span className="inline-flex rounded-full bg-[#F1F3F4] px-2.5 py-0.5 text-[9px] font-bold tracking-tight text-[#36404A]">{item.responseType}</span><p className="text-[13px] font-semibold leading-relaxed tracking-tight text-[#111827]">{item.response}</p><div className="flex flex-wrap items-center gap-3"><span className="text-[10px] font-medium tracking-tight text-[#66737F]">Reason: {item.reason}</span><button type="button" onClick={() => review(item)} className="text-[10px] font-bold tracking-tight text-[#0B74DE] hover:underline">View source detail</button></div></div></td>
              <td className="px-6 py-6"><div className="w-[220px] space-y-4"><span className={`inline-flex rounded-full px-2.5 py-0.5 text-[9px] font-bold tracking-tight ${item.stateTone}`}>{item.state}</span><p className="text-[12px] font-bold tracking-tight text-[#111827]">{item.stateSummary}</p></div></td>
              <td className="px-6 py-6"><div className="w-[240px] space-y-2"><p className="text-[13px] font-bold tracking-tight text-[#111827]">{item.proof}</p><p className="text-[11px] font-medium leading-relaxed text-[#6B7280]">{item.proofDetail}</p></div></td>
              <td className="px-6 py-6"><div className="w-[260px] space-y-2"><p className="text-[13px] font-semibold leading-relaxed tracking-tight text-[#111827]">{item.basis}</p><div className="text-[10px] font-medium tracking-tight text-[#66737F]">{item.updated}</div></div></td>
              <td className="px-6 py-6"><div className="w-[200px] space-y-3"><p className="text-[12px] font-bold leading-relaxed text-[#111827]">{item.next}</p><button type="button" onClick={() => review(item)} className="inline-flex h-8 w-full items-center justify-center gap-2 rounded-md bg-[#0B74DE] text-[12px] font-medium tracking-tight text-white shadow-none hover:bg-[#075EAF]">Review response<ArrowUpRight className="h-3.5 w-3.5" /></button></div></td>
              <td className="px-6 py-6"><ChevronRight className="h-4 w-4 text-[#E5E7EB] group-hover:translate-x-0.5 group-hover:text-[#111827]" /></td>
            </tr>)}</tbody>
          </table>
