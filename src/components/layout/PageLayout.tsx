...
export function PageLayout({
  ...
  return <div className={`min-h-screen h-full flex flex-col platform ${shouldShowMidnightBg ? 'relative bg-background' : plainBackground ? 'bg-white' : 'bg-[#F9FAFB] text-[#111827]'}`}>
    ...
    <div className={`flex-1 flex ${shouldShowMidnightBg ? 'bg-[#070707]' : 'bg-[#F9FAFB]'}`}>
      {!hideSidebar && (
        <Sidebar isCollapsed={isSidebarCollapsed} onToggle={toggleSidebar} />
      )}

      <main className={`flex-1 transition-all duration-300 ${mainIndent} overflow-hidden ${shouldShowMidnightBg ? 'bg-[#070707]' : 'bg-[#F9FAFB]'}`}>
